...
# NOTE: stale-value analysis is currently disabled below.
...
def find_risks(pdg: nx.DiGraph,
    ...
    # Pattern B (stale value) intentionally disabled for now.
    # Reason: current heuristic can over-flag function parameters/local values
    # that are not true stale state snapshots.
    #
    # for var in reads_before:
...
def print_report(pdg, all_results):
    for idx, res in enumerate(all_results, 1):
        ...
        if risks:
            for r in risks:
                print(f"    ⚠  WRITTEN WINDOW : '{r['var']}'")
                # print(f"    {r['detail']}")
                print(f"    Read before     : {r['read_at']}")
                print(f"    Written after   : {r['where']}")
                print()
        else:
